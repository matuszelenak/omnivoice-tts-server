...
async def create_speech(req: SpeechRequest):
    ...
    if req.stream_format == "sse":
        return StreamingResponse(
            _sse_stream(loop, synth, req.input, fmt),
            media_type="text/event-stream",
            headers={"Cache-Control": "no-store"},
        )

    samples = await loop.run_in_executor(_executor, partial(synth, req.input))
...
async def _sse_stream(loop, synth, text: str, fmt: str):
    ...
    for i, sentence in enumerate(split_to_sentences(text)):
        ...
        samples = await loop.run_in_executor(_executor, partial(synth, sentence, num_step=num_step))
        data = await loop.run_in_executor(_executor, partial(audio.encode, samples, fmt))
    ...
    def _process():
        ...
        try:
            for sentence in generate_sentences(_text_gen()):
                ...
                sentence_idx += 1
                samples = _executor.submit(partial(synth, sentence, num_step=num_step)).result()
